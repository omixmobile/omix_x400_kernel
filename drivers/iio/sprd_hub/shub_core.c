...
unsigned char gsensor_name_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
unsigned char gsensor_proc_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_gsensor_info_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < _HW_SENSOR_TOTAL;i++) {
		if(i == 0) {
			printk("%s is %s\n",SENSOR_TYPENAME[i],hw_sensor_id[i].pname);
			strcpy(gsensor_name_buffer, hw_sensor_id[i].pname);
		}
	}

	if(!strcmp(gsensor_name_buffer,"accelerometer_icm40607"))
		strcpy(gsensor_proc_buffer, "TDK");
	else if(!strcmp(gsensor_name_buffer,"accelerometer_icm42607"))
		strcpy(gsensor_proc_buffer, "TDK");
	else if(!strcmp(gsensor_name_buffer,"accelerometer_lsm6dsr"))
		strcpy(gsensor_proc_buffer, "ST");
	else
		strcpy(gsensor_proc_buffer, "unknown");

	seq_printf(m, "%s:%s\n", gsensor_proc_buffer, gsensor_name_buffer);
...
unsigned char gyrosensor_name_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
unsigned char gyrosensor_proc_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_gyrosensor_info_read(struct seq_file *m, void *v)
{
	int i;
	const char * const SENSOR_TYPENAME[] = {
		"acc", "mag", "gyro",
	...
	for (i = 0; i < _HW_SENSOR_TOTAL;i++) {
		if(i == 2) {
			printk("%s is %s\n",SENSOR_TYPENAME[i],hw_sensor_id[i].pname);
			strcpy(gyrosensor_name_buffer, hw_sensor_id[i].pname);
		}
	}

	if(!strcmp(gyrosensor_name_buffer,"gyroscope_icm40607"))
		strcpy(gyrosensor_proc_buffer, "TDK");
	else if(!strcmp(gyrosensor_name_buffer,"gyroscope_icm42607"))
		strcpy(gyrosensor_proc_buffer, "TDK");
	else if(!strcmp(gyrosensor_name_buffer,"gyroscope_lsm6dsr"))
		strcpy(gyrosensor_proc_buffer, "ST");
	else
		strcpy(gyrosensor_proc_buffer, "unknown");

	seq_printf(m, "%s:%s\n", gyrosensor_proc_buffer, gyrosensor_name_buffer);

    return 0;
};

static int proc_sprocomm_gyrosensor_info_open(struct inode *inode, struct file *file)
{
       return single_open(file, subsys_sprocomm_gyrosensor_info_read, NULL);
};

static const struct file_operations gyrosensor_proc_info_fops = {
       .owner = THIS_MODULE,
       .read = seq_read,
       .open = proc_sprocomm_gyrosensor_info_open,
};

unsigned char psensor_name_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
unsigned char psensor_proc_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_psensor_info_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < _HW_SENSOR_TOTAL;i++) {
		if(i == 3) {
			printk("%s is %s\n",SENSOR_TYPENAME[i],hw_sensor_id[i].pname);
			strcpy(psensor_name_buffer, hw_sensor_id[i].pname);
		}
	}

	if(!strcmp(psensor_name_buffer,"proximity_stk33562"))
		strcpy(psensor_proc_buffer, "Sensortek");
	else
		strcpy(psensor_proc_buffer, "unknown");
...
unsigned char lsensor_name_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
unsigned char lsensor_proc_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_lsensor_info_read(struct seq_file *m, void *v)
{
	int i;
	const char * const SENSOR_TYPENAME[] = {
		"acc", "mag", "gyro",
	...
	for (i = 0; i < _HW_SENSOR_TOTAL;i++) {
		if(i == 4) {
			printk("%s is %s\n",SENSOR_TYPENAME[i],hw_sensor_id[i].pname);
			strcpy(lsensor_name_buffer, hw_sensor_id[i].pname);
		}
	}

	if(!strcmp(lsensor_name_buffer,"light_stk33562"))
		strcpy(lsensor_proc_buffer, "Sensortek");
	else
		strcpy(lsensor_proc_buffer, "unknown");

	seq_printf(m, "%s:%s\n", lsensor_proc_buffer, lsensor_name_buffer);

    return 0;
};

static int proc_sprocomm_lsensor_info_open(struct inode *inode, struct file *file)
{
       return single_open(file, subsys_sprocomm_lsensor_info_read, NULL);
};

static const struct file_operations alssensor_proc_info_fops = {
       .owner = THIS_MODULE,
       .read = seq_read,
       .open = proc_sprocomm_lsensor_info_open,
};

unsigned char magsensor_name_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
unsigned char magsensor_proc_buffer[SENSOR_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_magsensor_info_read(struct seq_file *m, void *v)
{
	int i;
	const char * const SENSOR_TYPENAME[] = {
		"acc", "mag", "gyro",
	...
	for (i = 0; i < _HW_SENSOR_TOTAL;i++) {
		if(i == 1) {
			printk("%s is %s\n",SENSOR_TYPENAME[i],hw_sensor_id[i].pname);
			strcpy(magsensor_name_buffer, hw_sensor_id[i].pname);
		}
	}

	if(!strcmp(magsensor_name_buffer,"magnetic_qmc6308"))
		strcpy(magsensor_proc_buffer, "QST");
	else
		strcpy(magsensor_proc_buffer, "unknown");

	seq_printf(m, "%s:%s\n", magsensor_proc_buffer, magsensor_name_buffer);

    return 0;
};

static int proc_sprocomm_maggsensor_info_open(struct inode *inode, struct file *file)
{
       return single_open(file, subsys_sprocomm_magsensor_info_read, NULL);
};

static const struct file_operations magsensor_proc_info_fops = {
       .owner = THIS_MODULE,
       .read = seq_read,
       .open = proc_sprocomm_maggsensor_info_open,
};
...
static int shub_probe(struct platform_device *pdev)
{
	...
	proc_create("Sprocomm_GyroSensorInfo", 0664, NULL, &gyrosensor_proc_info_fops);
	proc_create("Sprocomm_PSensorInfo", 0664, NULL, &psensor_proc_info_fops);
	proc_create("Sprocomm_ALSSensorInfo", 0664, NULL, &alssensor_proc_info_fops);
	proc_create("Sprocomm_MAGSensorInfo", 0664, NULL, &magsensor_proc_info_fops);
