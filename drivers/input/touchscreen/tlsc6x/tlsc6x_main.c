...
static struct wakeup_source gesture_timeout_wakelock;
#endif
#endif
...
static ssize_t virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d\n",
			...
			pdata->virtualkeys[0], pdata->virtualkeys[1], pdata->virtualkeys[2], pdata->virtualkeys[3],
			__stringify(EV_KEY), __stringify(KEY_HOMEPAGE),
			pdata->virtualkeys[4], pdata->virtualkeys[5], pdata->virtualkeys[6], pdata->virtualkeys[7],
			__stringify(EV_KEY), __stringify(KEY_APPSELECT),
			pdata->virtualkeys[8], pdata->virtualkeys[9],pdata->virtualkeys[10], pdata->virtualkeys[11]);
}
...
#if 1
        __pm_wakeup_event(&gesture_timeout_wakelock, msecs_to_jiffies(2000));
#endif
...
{
	return sprintf(buf, "%d\n", (real_suspend_flag == 1) ? "true" : "false");
}
...
static int tp_sysfs_init(struct tlsc6x_data *drvdata)
{
	int ret = 0;
	tp_ctrl_kobj = kobject_create_and_add("touchscreen", NULL);
	if (!tp_ctrl_kobj){
		tlsc_err("Create /sys/touchscreen failed!");
        return -ENOMEM;
	}else
		tlsc_info("Create /sys/touchscreen succesfully!");

	ret = sysfs_create_group(tp_ctrl_kobj, &tp_attr_group);
	if(ret){
		tlsc_err("Create /sys/touchscreen/ts_suspend failed!");
		kobject_put(tp_ctrl_kobj);
	}else
		tlsc_info("Create /sys/touchscreen/ts_suspend succesfully!");
...
static struct tlsc6x_platform_data *tlsc6x_parse_dt(struct device *dev)
{
    ...
    if (ret)
		dev_err(dev,"tpd-firmware-update undefined!");

	ret=of_property_read_u32(np, "have-virtualkey", &pdata->have_virtualkey);
	if (ret)
		dev_err(dev,"have-virtualkey undefined!");
...
//	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, 127, 0, 0);
...
#ifdef TP_GESTRUE
#if 1 //(LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0))
	gesture_timeout_wakelock = *(wakeup_source_register(NULL,"gesture_timeout_wakelock"));
#endif
	...
	tlsc6x_wakelock = *(wakeup_source_register(NULL, "tlsc6x_wakelock"));
	...
	err = tp_sysfs_init(g_tp_drvdata);
	if(err){
		tlsc_err("create suspend sysfs node faile");
	}
