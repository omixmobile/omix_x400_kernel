        ...
        u32 addr, u8 *b, int retry, int delay)
{
    ...
    cts_dbg("Readb from slave_addr: 0x%02x address width: %u reg: 0x%0*x",
        ...
        u32 addr, u16 *w, int retry, int delay)
{
    ...
    cts_dbg("Readw from slave_addr: 0x%02x address width: %u reg: 0x%0*x",
        cts_dev->rtdata.slave_addr, cts_dev->rtdata.addr_width * 2, addr);
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
...
#ifdef CFG_CTS_UPDATE_CRCCHECK
	if (cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911S || cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911C) {
		auto_boot = 1;
	}
#endif
	for(retries = 5; retries >= 0; retries--) {
		...
		if (ret) {
			...
			if (fwid == CTS_DEV_FWID_ICNL9911 || fwid == CTS_DEV_FWID_ICNL9911S || fwid == CTS_DEV_FWID_ICNL9911C)
			{
...
static ssize_t sprocomm_read_tp_info(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	strcpy(tp_module_buffer, "LIANCHUANG");
	strcpy(tp_proc_buffer, "ICNL9911C");
