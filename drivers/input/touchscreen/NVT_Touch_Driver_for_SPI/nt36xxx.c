...
static bool boot_mode_charger = false;

static int boot_mode_check(char *str)
{
	if (str != NULL && !strncmp(str, "cali", strlen("cali")))
		calibration_mode = true;
	else if(str != NULL &&  !strncmp(str,"charger",strlen("charger")))
		boot_mode_charger = true;
	else{
		calibration_mode = false;
		boot_mode_charger = false;
	}
	return 0;
}
...
#if WAKEUP_GESTURE
extern int sprocomm_gesture_enable;
static ssize_t sprocomm_gesture_enable_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	s32 ret = 0;
    unsigned char nUserTempBuffer[2]= {0};

	ret = copy_from_user(nUserTempBuffer, buffer, count);
    if (ret) {
		return -EPERM;
    }

	sprocomm_gesture_enable = nUserTempBuffer[0] == '1'?1:0;
	return count;
}

static ssize_t sprocomm_gesture_enable_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
    unsigned char nUserTempBuffer[16];
    ...
    length = sprintf(nUserTempBuffer, "%d\n", sprocomm_gesture_enable);
    if(copy_to_user(buffer, nUserTempBuffer, length))
    {
		return -EFAULT;
    }
...
static struct file_operations sprocomm_gesture_enable_fops = {
	.owner = THIS_MODULE,
	.write = sprocomm_gesture_enable_write,
	.read = sprocomm_gesture_enable_read,
};

static int sprocomm_creat_proc_gesture_enable(void)
{
	struct proc_dir_entry *sprocomm_gesture_enable = NULL;
	sprocomm_gesture_enable = proc_create("gesture_enable", 0666, NULL, &sprocomm_gesture_enable_fops);
	if (sprocomm_gesture_enable == NULL)
    {
        printk("Create double_tap_enable fail\n");
    }
	return 0;
}
#endif
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		nvt_update_firmware_checkmodule();
...
return:
	n.a.
*******************************************************/
extern int tp_sprocomm_esd_flag;
...
/*
	...
	printk("\n");
*/
    if(tp_sprocomm_esd_flag){
        printk("nvt_ts tp_sprocomm_esd_flag = %d, tp esd check begin!",tp_sprocomm_esd_flag);
        tp_sprocomm_esd_flag =0;
        //nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME);
		nvt_update_firmware_checkmodule();
        nvt_irq_enable(true);
    }
...
static int tp_sysfs_init(struct nvt_ts_data *ts)
{
	int ret = 0;
	tp_ctrl_kobj = kobject_create_and_add("touchscreen", NULL);
	if (!tp_ctrl_kobj){
		NVT_ERR("Create /sys/touchscreen failed!");
        return -ENOMEM;
	}else
		NVT_LOG("Create /sys/touchscreen succesfully!");

	ret = sysfs_create_group(tp_ctrl_kobj, &tp_attr_group);
	if(ret){
		NVT_ERR("Create /sys/touchscreen/ts_suspend failed!");
		kobject_put(tp_ctrl_kobj);
	}else
		NVT_LOG("Create /sys/touchscreen/ts_suspend succesfully!");

	return ret;
}
#define TP_PROC_BUFFER_MAX_SIZE        64
unsigned char nvt_tp_proc_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};
unsigned char nvt_tp_module_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};

static int subsys_sprocomm_tp_info_read(struct seq_file *m, void *v)
{
       extern u16 nvt_tpver;
	   if(nt36525b_is_xx_module()==1||nt36525b_is_xx_module()==2){
		   strcpy(nvt_tp_proc_buffer, "HUAXIAN");
	   }else{
		   strcpy(nvt_tp_proc_buffer, "XINLI");
	   }       
       strcpy(nvt_tp_module_buffer, "NT36525B");
       NVT_LOG("%s %s:%s:0x%04x\n", __func__, nvt_tp_proc_buffer,nvt_tp_module_buffer,nvt_tpver);
       seq_printf(m, "%s:%s:0x%02x\n", nvt_tp_proc_buffer,nvt_tp_module_buffer,nvt_tpver);
       return 0;
};

static int proc_sprocomm_tp_info_open(struct inode *inode, struct file *file)
{
       return single_open(file, subsys_sprocomm_tp_info_read, NULL);
};

static struct file_operations tp_proc_fops_get_info = {
       .owner = THIS_MODULE,
       .read = seq_read,
       .open = proc_sprocomm_tp_info_open,
};
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if(tp_is_inited()){
          pr_info("nvt: Another touch screen has been inited!\n");
          return -ENODEV;
    }
	NVT_LOG("start\n");
	...
	nvt_irq_enable(true);
	tp_set_inited(true);
	ret = tp_sysfs_init(ts);
	proc_create("sprocomm_tpInfo", 0664, NULL, &tp_proc_fops_get_info);
	if(ret){
		NVT_ERR("create suspend sysfs node faile");
	}
	if(calibration_mode){
		NVT_LOG("calimode return\n");
		nvt_ts_suspend(&ts->client->dev);
	}

	sprocomm_creat_proc_gesture_enable();
	return 0;
...
int32_t nvt_ts_suspend_esd(void)
{
...
int32_t nvt_ts_resume_esd(void)
{
...
return:
	Executive outcomes. 0---succeed.
*******************************************************/
extern unsigned int bl_is_on;
int32_t nvt_ts_resume(struct device *dev)
{
	NVT_LOG("bl_is_on=%d,bTouchIsAwake = %d\n",bl_is_on,bTouchIsAwake);
	if(bl_is_on ==0){
		if (bTouchIsAwake) {
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	if (nvt_update_firmware_checkmodule()) {
