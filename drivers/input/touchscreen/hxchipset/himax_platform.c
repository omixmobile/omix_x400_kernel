...
static struct himax_ts_data *ts;
...
int himax_dev_set(struct himax_ts_data *ts)
{
	...
	ts->input_dev->name = "adaptive_ts_himax-touchscreen";
	...
	ts->hx_pen_dev->name = "adaptive_ts_himax-pen";
skip_pen_operation:
		...
		struct himax_i2c_platform_data *pdata)
{
	...
	if (ret == 0) {
		...
		printk(" DT-%s:panel-coords = %d, %d, %d, %d\n", __func__,
				pdata->abs_x_min,
				pdata->abs_x_max,
				pdata->abs_y_min,
				pdata->abs_y_max);
	...
	if (prop) {
		...
		if (coords_size != 4)
			printk(" %s:Invalid display coords size %d\n",
				__func__, coords_size);
	}
	...
	printk(" DT-%s:display-coords = (%d, %d)\n", __func__,
			pdata->screenWidth,
			pdata->screenHeight);
	...
	if (!gpio_is_valid(pdata->gpio_3v3_en))
		printk(" DT:gpio_3v3_en value is not valid\n");

	printk(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d\n",
			pdata->gpio_irq,
			pdata->gpio_reset,
			pdata->gpio_3v3_en);
...
static ssize_t ts_suspend_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (input == 1){
	  printk("himax ts_suspend_store himax_chip_common_suspend\n");
      himax_chip_common_suspend(ts);
	}
    else if (input == 0){
		 printk("himax ts_suspend_store himax_chip_common_resume\n");
         himax_chip_common_resume(ts);   
	}		 
...
static int tp_sysfs_init(struct himax_ts_data *ts)
{
        int ret = 0;
        tp_ctrl_kobj = kobject_create_and_add("touchscreen", NULL);
        if (!tp_ctrl_kobj){
                printk("Create /sys/touchscreen failed!");
        return -ENOMEM;
        }else
                printk("Create /sys/touchscreen succesfully!");

        ret = sysfs_create_group(tp_ctrl_kobj, &tp_attr_group);
        if(ret){
                printk("Create /sys/touchscreen/ts_suspend failed!");
                kobject_put(tp_ctrl_kobj);
        }else
		{
                printk("Create /sys/touchscreen/ts_suspend succesfully!");
		}
...
unsigned char hx_tp_proc_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};
unsigned char hx_tp_module_buffer[TP_PROC_BUFFER_MAX_SIZE]={0};
 
static int subsys_sprocomm_tp_info_read(struct seq_file *m, void *v)
{
       
       strcpy(hx_tp_proc_buffer, "TRULY");
       strcpy(hx_tp_module_buffer, "HX83102D");
       printk("%s %s:%s:0x%04x\n", __func__, hx_tp_proc_buffer,hx_tp_module_buffer,ic_data->vendor_touch_cfg_ver);
       seq_printf(m, "%s:%s:0x%02x\n", hx_tp_proc_buffer,hx_tp_module_buffer,ic_data->vendor_touch_cfg_ver);
       return 0;
};
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	if(calibration_mode){
		printk("himax_chip_common_probe calimode return\n");
		return -ENOMEM;
	}
	printk("Enter %s\n", __func__);
	...
	if(ret){
		 printk("create suspend sysfs node faile\n");
	}
	if (ret < 0)
...
{
	printk("Himax check double loading\n");
	if (g_mmi_refcnt++ > 0) {
		printk("Himax driver has been loaded! ignoring....\n");
		return 0;
	}
