...
#if defined(__EMBEDDED_FW__)
		image = &g_embedded_fw;
		printk("%s: Not find FW in userspace, use embedded FW(size:%zu)",
...
UPDATE_FW:
	if (i_update_FW() <= 0)
		printk("%s: himax Update FW fail\n", __func__);
	else
		printk("%s: himax Update FW success\n", __func__);
...
int himax_chip_common_init(void)
{
	...
	printk("himax Prepare kernel fp\n");
	...
	if (idx >= 0) {
		if (isEmpty(idx) != 0) {
			printk("himax %s: no chip registered, please insmod ic.ko!\n",
					__func__);
		...
		for (i = 0; i < entry->hx_ic_dt_num; i++) {
			if (entry->core_chip_dt[i].fp_chip_detect != NULL) {
				if (entry->core_chip_dt[i].fp_chip_detect()) {
					printk("himax %s: chip found! list_num=%d\n",
							__func__, i);
					goto found_hx_chip;
				} else {
					printk("himax %s:num=%d,chip NOT found! go Next\n",
							__func__, i);
					continue;
				}
			}
		...
		printk("himax %s: No available chip exist!\n", __func__);
		goto error_ic_detect_failed;
	}

	if (i == entry->hx_ic_dt_num) {
		printk("himax %s: chip detect failed!\n", __func__);
		goto error_ic_detect_failed;
	}
...
found_hx_chip:
	...
	printk("himax fp_read_FW_ver START\n");
	g_core_fp.fp_read_FW_ver();
	hx_chk_flash_sts();
#endif
...
#if defined(HX_AUTO_UPDATE_FW)
FW_force_upgrade:
    printk("himax FW_force_upgrade START1111 \n");
	ts->himax_update_wq =
...
{
	if (ts->suspended) {
		printk("%s: Already suspended. Skipped.\n", __func__);
		goto END;
	} else {
		ts->suspended = true;
		printk("%s: enter\n", __func__);
