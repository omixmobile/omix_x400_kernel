...
void ili_tp_reset_control(int value)
{
	if(!ilits)
		return;
	ILI_INFO("%s %d\n", __func__, value);
...
EXPORT_SYMBOL(ili_tp_reset_control);
...
{
	extern u32 ili_tpver[];
	strcpy(tp_proc_buffer, "HUAYING");
	strcpy(tp_module_buffer, "ILI9882Q");
	ILI_INFO("%s %s:%s:%d.%d.%d.%d\n", __func__, tp_proc_buffer,tp_module_buffer,ili_tpver[0],ili_tpver[1],ili_tpver[2],ili_tpver[3]);
	seq_printf(m, "%s:%s:%d.%d.%d.%d\n", tp_proc_buffer,tp_module_buffer,ili_tpver[0],ili_tpver[1],ili_tpver[2],ili_tpver[3]);
	return 0;
};
