...
void ilitek_tddi_wq_ctrl(int type, int ctrl)
{
	switch (type) {
	case WQ_ESD:
		if (idev->esd_func_ctrl || idev->wq_ctrl) {
			if (!esd_wq) {
...
#if AXIS_PACKET
		len += P5_X_DEMO_MODE_PACKET_INFO_LEN + P5_X_DEMO_MODE_AXIS_LEN + P5_X_DEMO_MODE_STATE_INFO;
#endif
...
int ilitek_tddi_report_handler(void)
{
	...
	if (!idev->boot || !idev->report || atomic_read(&idev->tp_reset) ||
	...
	case P5_X_DEMO_AXIS_PACKET_ID:
		ilitek_tddi_report_ap_mode(&trdata[P5_X_DEMO_MODE_PACKET_INFO_LEN], (rlen-P5_X_DEMO_MODE_PACKET_INFO_LEN - P5_X_DEMO_MODE_AXIS_LEN - P5_X_DEMO_MODE_STATE_INFO));
		break;
...
#if AXIS_PACKET
	idev->tp_data_len += P5_X_DEMO_MODE_PACKET_INFO_LEN + P5_X_DEMO_MODE_AXIS_LEN + P5_X_DEMO_MODE_STATE_INFO;
#endif
...
int ilitek_tddi_init(void)
{
	...
	if (ilitek_tddi_ic_get_info() < 0){
		ipio_err("Chip info is incorrect its not V2\n");
		return -ENODEV;
	}
...
void ilitek_tddi_dev_remove(bool flag)
{
	...
	kfree(idev->tr_buf);
	kfree(idev->gcoord);
	ilitek_tddi_interface_dev_exit(idev, flag);
}
