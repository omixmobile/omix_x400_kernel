...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (idev->dbl[idev->odi].mark) {
		...
		}  else if (idev->dbl[idev->odi].data[0] == P5_X_DEBUG_AXIS_PACKET_ID) {
			send_data_len = 0;
			need_read_data_len = TR_BUF_SIZE - 8;
		}
...
static ssize_t ilitek_proc_sram_test_info(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ret = ilitek_tddi_reset_ctrl(TP_IC_WHOLE_RST);
	if (ret < 0) {
		ipio_err("TP_IC_WHOLE_RST Failed, ret = %d\n", ret);
	...
	ret = ilitek_tddi_fw_upgrade_handler(NULL);
	if (ret < 0) {
		ipio_err("fw upgrade Failed, ret = %d\n", ret);
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "%s\n", "fw upgrade Failed");
	}
...
{
	int ret = 0;
	ipio_info("set mp lcm off env to %d\n", lcm_on);
	if (lcm_on == ENABLE) {
		if (ENABLE_WQ_ESD)
			idev->esd_func_ctrl = ENABLE;
