...
int ilitek_tddi_ic_sram_test(void)
{
	int retry = 20;
	u32 bist_done = 0, bist_fail = 0, bist_fail_bus = 0, inbist_failbus = 0;

	ilitek_ice_mode_write(0x040004, 0x01, 1);
	ilitek_ice_mode_write(0x040010, 0x9878, 2);
	ilitek_ice_mode_write(0x047003, 0x00, 1);
	ilitek_ice_mode_write(0x04001C, 0x01, 1);

	ilitek_ice_mode_write(0x071001, 0x02, 1);
	ilitek_ice_mode_write(0x071005, 0x3C, 1);

	ilitek_ice_mode_write(0x040088, 0x6B8A, 2);

	ilitek_ice_mode_write(0x046003, 0x00, 1);

	ilitek_ice_mode_write(0x046000, 0x010001, 3);

	ilitek_ice_mode_write(0x046002, 0x00, 1);

	switch (idev->chip->id) {
	case ILI9881_CHIP:
		ilitek_ice_mode_write(0x046004, 0x1FFFFF00, 4);
		break;
	case ILI9882_CHIP:
		ilitek_ice_mode_write(0x046004, 0x1FFFFF00, 4);
		break;
	...
	ilitek_ice_mode_write(0x046014, 0xA4409800, 4);
	ilitek_ice_mode_write(0x046018, 0xB480F590, 4);
	ilitek_ice_mode_write(0x04601C, 0x9000F560, 4);
	ilitek_ice_mode_write(0x046020, 0x9280C460, 4);
	ilitek_ice_mode_write(0x046024, 0x0000, 2);

	ilitek_ice_mode_write(0x046000, 0x000000, 4);
	ilitek_ice_mode_read(0x04600C, &bist_done, sizeof(bist_done));
	ilitek_ice_mode_read(0x04600D, &bist_fail, sizeof(bist_fail));
	ilitek_ice_mode_read(0x046010, &bist_fail_bus, sizeof(bist_fail_bus));

	if (((bist_done & BIT(0)) != 0x00) || ((bist_fail & BIT(0)) != 0x01) || (bist_fail_bus != 0xFFFFFFFF)) {
		ipio_debug("SRAM TEST FAIL\n");
		return -1;
	}

	ilitek_ice_mode_write(0x046000, 0x00000001, 4);
	ilitek_ice_mode_write(0x046000, 0x00000101, 4);
	...
	ilitek_ice_mode_read(0x04600D, &bist_fail, sizeof(bist_fail));
	ilitek_ice_mode_read(0x046010, &inbist_failbus, sizeof(inbist_failbus));
...
int ilitek_tddi_ic_get_info(void)
{
	...
	if (idev->chip->pid != 0x98811115)
	{
		ipio_err("shuangdan  its not ili9881h\n");
		return -1 ;
	}
