...
unsigned char tp_proc_buffer_v2[TP_PROC_BUFFER_MAX_SIZE]={0};
unsigned char tp_module_buffer_v2[TP_PROC_BUFFER_MAX_SIZE]={0};
static int subsys_sprocomm_tp_info_read(struct seq_file *m, void *v)
{
	extern u32 ili_tpver_v2[];
	strcpy(tp_proc_buffer_v2, "HUAYING");
	strcpy(tp_module_buffer_v2, "ILI9881H");
	ipio_info("%s %s:%s:%d.%d.%d.%d\n", __func__, tp_proc_buffer_v2,tp_module_buffer_v2,ili_tpver_v2[0],ili_tpver_v2[1],ili_tpver_v2[2],ili_tpver_v2[3]);
	seq_printf(m, "%s:%s:%d.%d.%d.%d\n", tp_proc_buffer_v2,tp_module_buffer_v2,ili_tpver_v2[0],ili_tpver_v2[1],ili_tpver_v2[2],ili_tpver_v2[3]);
	return 0;
};
...
static int ilitek_plat_probe(void)
{
	...
	if (tp_is_inited()){
		ipio_err("another tp has been inited\n");
		return -ENODEV;
	}
