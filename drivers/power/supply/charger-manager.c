...
#define OMIX_FAST_CHARGE
#ifdef OMIX_FAST_CHARGE
#include <linux/proc_fs.h>
#include <linux/uaccess.h>
#include <linux/seq_file.h>
static int sprocomm_fast_charge_enable = 1;
#endif
...
#ifdef OMIX_FAST_CHARGE
	if(!sprocomm_fast_charge_enable)
		return false;
#endif
...
#ifdef OMIX_FAST_CHARGE
static ssize_t sprocomm_fast_charge_enable_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	s32 ret = 0;
	...
	sprocomm_fast_charge_enable = nUserTempBuffer[0] == '1'?1:0;
	printk("sprocomm fast_charge_enable set %d\n", sprocomm_fast_charge_enable);
	return count;
}

static ssize_t sprocomm_fast_charge_enable_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	u32 length = 0;
...
static struct file_operations sprocomm_fast_charge_enable_fops = {
	.owner = THIS_MODULE,
	.write = sprocomm_fast_charge_enable_write,
	.read = sprocomm_fast_charge_enable_read,
};

static int sprocomm_creat_proc_fast_charge_enable(void)
{
	struct proc_dir_entry *sprocomm_proc_fast_charge_enable = NULL;
	sprocomm_proc_fast_charge_enable = proc_create("fast_charge_enable", 0666, NULL, &sprocomm_fast_charge_enable_fops);
	if (!sprocomm_proc_fast_charge_enable)
		printk("sprocmm Create fast_charge_enable node fail\n");
...
#ifdef OMIX_FAST_CHARGE
	sprocomm_creat_proc_fast_charge_enable();
#endif
