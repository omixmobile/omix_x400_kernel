...
#define SPROCOMM_BATTREY_TEMP
#ifdef SPROCOMM_BATTREY_TEMP
#include <linux/proc_fs.h>  /*proc*/
int sprocomm_battery_temp = 250;
#endif

#define SPROCOMM_TEMP_DEBUG
#ifdef SPROCOMM_TEMP_DEBUG
static unsigned int Temp_Fix = 0;
static int Fixed_Temp = 250;
#endif
...
static int sc27xx_fgu_get_average_temp(struct sc27xx_fgu_data *data, int temp)
{
	...
	sum = (sum - max - min) / (SC27XX_FGU_TEMP_BUFF_CNT - 2);

#ifdef SPROCOMM_TEMP_DEBUG
	if(Temp_Fix == 1){
		sum = Fixed_Temp;
		dev_info(data->dev, "Temp_Fix = 1!, set temp to %d\n", Fixed_Temp);
	}
#endif

#ifdef SPROCOMM_BATTREY_TEMP
	sprocomm_battery_temp = sum;
#endif
...
#ifdef SPROCOMM_BATTREY_TEMP
static ssize_t sprocomm_read_battery_temp(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
       char *page = NULL;
       char *ptr = NULL;
       int len = 0;
...
static const struct file_operations battery_temp_fops = {
    .read = sprocomm_read_battery_temp
};


static int sprocomm_creat_proc_battery_temp(void)
{
       struct proc_dir_entry *battery_temp_entry = NULL;

       battery_temp_entry = proc_create("sprocomm_battery_temp", 0444, NULL, &battery_temp_fops);
       if (battery_temp_entry == NULL){
		   printk("Create sprocomm_battery_temp \n");
		}
       return 0;
...
#ifdef SPROCOMM_TEMP_DEBUG
static ssize_t ntc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
       return sprintf(buf, "%d\n", Temp_Fix);
}

static ssize_t ntc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	ret = kstrtoint(buf, 10, &Temp_Fix);
...
{
	int ret = 0;
	ret = kstrtoint(buf, 10, &Fixed_Temp);

	return size;
}

DEVICE_ATTR(ntc, 0644, ntc_show, ntc_store);
...
#ifdef SPROCOMM_BATTREY_TEMP
	sprocomm_creat_proc_battery_temp();
#endif
#ifdef SPROCOMM_TEMP_DEBUG
	device_create_file(&pdev->dev, &dev_attr_ntc);
